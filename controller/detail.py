...
class Detail:
    ...
    def update_process(self, wb):
        for _ in range(12):
            ...
            i.check_stop_signal()
            try:
                task = GB.redis.dequeue(GB.process_cache_conf['detail.retry']['key'])
                if task is None:
                    time.sleep(random.randint(900, 1200))
                    break
                vid = int(task)
                record = self.session.query(SourceVideoModel).filter(
                    SourceVideoModel.id == vid).first()
                if record is None:
                    continue
                i.insert_current_task('img', f'详情重导入《{record.title}》-{record.source_url}')
                GB.redis.delete(GB.process_cache_conf['hook_video']['key'])
                GB.redis.delete(GB.process_cache_conf['hook_cover']['key'])
                ...
                self.sync_receive_info(receiver,
                                       video=GB.process_cache_conf['hook_video']['key'],
                                       cover=GB.process_cache_conf['hook_cover']['key'])
                self.session.query(SourceVideoModel).filter(SourceVideoModel.id == vid).update({
                    SourceVideoModel.url: receiver['video'],
                    SourceVideoModel.big_cover: receiver['cover']})
                self.session.commit()
            except Exception as e:
